...
void CrankNicolson::initialise() {
	...
	double* values = new double[num_mat_entries];
	// column indices for sparse matrix representation
	int* col_ind = new int[num_mat_entries];
	// row pointer for sparse matrix representation
	int* row_ptr = new int[num_voxels+1];
...
//	exit(1)
	...
	lhs = dimnew CompRow_Mat_double(num_voxels, num_voxels, num_mat_entries, &values[0], &row_ptr[0], &col_ind[0]);
