...
*/
int NF::nfmain(int argc, char *argv[])
{
	...
	if(NF::parseArguments(argc, const_cast<const char**>(argv), argMap))
	{
		...
		if(argMap.find("seed")!= argMap.end()) {
			int seed = abs(NF::parseAsInt(argMap,"seed",0));
...
bool NF::runRNFscript(map<string,string> argMap, bool verbose)
{
	//Step 1: open the file and initialize the argMap
	vector<string> commands;
	if(!NF::readRNFfile(argMap, commands, verbose)) {
	...
	if(s!=0) {
		...
		bool output = NF::runRNFcommands(s,argMap,commands,verbose);
...
System *NF::initSystemFromFlags(map<string,string> argMap, bool verbose)
{
	...
	if (argMap.find("xml")!=argMap.end())
	{
		...
		if(!filename.empty())
		{
			...
			if (argMap.find("gml")!=argMap.end()) {
				globalMoleculeLimit = NF::parseAsInt(argMap,"gml",globalMoleculeLimit);
			}

			//Actually create the system
			bool cb = false;
			if(turnOnComplexBookkeeping || blockSameComplexBinding) cb=true;
			System *s = NF::initializeFromXML(filename,cb,globalMoleculeLimit,verbose);
			...
			if(s!=NULL)
			{
				...
				if (argMap.find("dump")!=argMap.end()) {
					if(!NF::createSystemDumper(argMap.find("dump")->second, s, verbose)) {
				...
				if (argMap.find("utl")!=argMap.end()) {
					int utl = -1;
					utl = NF::parseAsInt(argMap,"utl",utl);
...
bool NF::runFromArgs(System *s, map<string,string> argMap, bool verbose)
{
	...
	else
	{
		...
		eqTime = NF::parseAsDouble(argMap,"eq",eqTime);
		sTime = NF::parseAsDouble(argMap,"sim",sTime);
		oSteps = NF::parseAsInt(argMap,"oSteps",(int)oSteps);
...
void NF::printLogo(int indent, string version)
{
	string s;
...
void NF::printHelp(string version)
{
