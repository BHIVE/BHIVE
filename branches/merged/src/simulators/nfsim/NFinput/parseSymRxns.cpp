...
bool NF::FindReactionRuleSymmetry(
			...
			for(unsigned int k=0; k<offRxnCenterSymClasses.at(mIndex).size(); k++) {
				...
				if(c.name.compare(potentialSymComp.name)==0) {
					if(c.stateConstraintLabel.compare(potentialSymComp.stateConstraintLabel)==0) {
						...
						int numOfBondsInt = -1;
						try {
							numOfBondsInt = Util::convertToInt(c.numOfBondsLabel);
...
bool NF::generateRxnPermutations(vector<map<string,component> > &permutations,
		map<string,component> &symComps,
		map<string,component> &symRxnCenter,
		bool verbose)
{
...
bool NF::lookup(component *&c, string id, map<string,component> &comps, map<string,component> &symMap) {
	try {
...
bool NF::readPatternForSymmetry(
