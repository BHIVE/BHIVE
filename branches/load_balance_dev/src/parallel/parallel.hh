...
void setmpicoord(int i, int c);

void parallel_initialize(int* argc, char**argv[],int& inrank, int& insize);

void parallel_finalize();

void parallel_abort(int code, string message = "Hive::abort");
...
void parallel_isend(int dest, char* data, int datalen, int tag = 0);

void parallel_irecv(int source, int tag = 0);

void parallel_waitall(char*& data, int& datalen);
