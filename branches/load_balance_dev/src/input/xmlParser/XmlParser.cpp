...
Agent* XmlParser::parseAgentNode(TiXmlNode* node, vector<Agent*>& orchestra)
{
    ...
    // lookup node tag in parsers
    if ( value != NULL && this->tagParsers.count(value) > 0 ) {
        // if found, make sure xml is valid
        TiXmlElement* element = node->ToElement();
        if ( element == NULL ) {
            cerr << "Element " << value << " invalid!" << endl;
            return (Agent *) NULL;
        }
        ...
        AgentFactory* nodeAgentFactory = this->tagParsers[value];
...
bool XmlParser::isAgentNode(TiXmlNode* node)
{
    ...
    if ( !(value.empty()) &&
        this->tagParsers.count(element->Value()) > 0 ) {
        return true;
    } else { return false; }
}
