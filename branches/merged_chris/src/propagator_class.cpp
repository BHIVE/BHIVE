...
void PropagatorClass::go(double dt, double endTime) {
	...
	//Setting observables for each agent in the output writer
	for (it = agentMap_.begin(); it != agentMap_.end(); ++it) {
		it->second->getDatabase()->setDBObservables(getOutputWriter());
	}
	for (; currentTime_ < endTime; currentTime_ += dt) {
		...
		// STEP 3.5: Dumping agent data to the output writer for printing to disk
		for (it = agentMap_.begin(); it != agentMap_.end(); ++it) {
			it->second->getDatabase()->printDBObservables(getOutputWriter());
		}
		//Printing the output buffers
		getOutputWriter()->printBuffers();
	...
	//Forcing the output writer to clear its buffers
	getOutputWriter()->printBuffers(true);
}
...
void PropagatorClass::removeAgent(int id) {
    ...
    // cerr << "removing agent id = " << id << endl;
