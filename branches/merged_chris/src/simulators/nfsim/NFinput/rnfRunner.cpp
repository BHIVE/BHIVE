...
bool NF::readRNFfile(map<string,string> &argMap, vector<string> &commands, bool verbose)
{
...
void simulate(string command,System *s, bool verbose)
{
	...
	Util::trim(simTime_str);
	Util::trim(oSteps_str);

	try {
		simTime=Util::convertToDouble(simTime_str);
		oSteps=Util::convertToInt(oSteps_str);
		cout<<">> ";
...
void setParameter(string command, System *s) {
	...
	Util::trim(paramName);
	Util::trim(paramValue_str);
	try {
		paramValue=Util::convertToDouble(paramValue_str);
...
bool NF::runRNFcommands(System *s, map<string,string> &argMap, vector<string> &commands, bool verbose)
{
