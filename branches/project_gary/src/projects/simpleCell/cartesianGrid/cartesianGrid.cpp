...
void CartesianGrid::printDBObservables(OutputWriter *ow) {
	if (observableIDs.size() == 0) {
		observableIDs.push_back(ow->getObservableIndex("concentration"));
	}
	vector<int> ids(4);
	vector<double> printData(1);
		...
		for (int j=0; j < xSize(); j++) {
			...
			for (int k=0; k < ySize(); k++) {
				...
				for (int m=0; m < zSize(); m++) {
					ids[3] = m;
					printData[0] = getVoxel(j,k,m)->compoundConcentrations[i];
...
void CartesianGrid::setDBObservables(OutputWriter *ow) {
	vector<string> ids;
	ids.push_back("compound");
	ids.push_back("x");
	ids.push_back("y");
	ids.push_back("z");
	observableIDs.push_back(ow->addObservable("concentration","Concentrations.txt",ids,0.05,true));
};

void CartesianGrid::printCellLocations() {
    return;
    
    for( list<CellWorldData*>::iterator IT = cellList.begin();
        IT != cellList.end(); IT++) {
...
CartesianGrid::~CartesianGrid() {
    ...
    for (map<int, CellWorldData*>::iterator it = cellData.begin(); it != cellData.end(); ++it) {
        removeCell(it->first);
    }
