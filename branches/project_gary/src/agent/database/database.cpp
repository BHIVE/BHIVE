...
void Database::printDBObservables(OutputWriter *ow) {
	for(unsigned d=0; d<real_database.size(); d++) {
		real_database[d]->printDBObservables(ow);
	}
}

void Database::setDBObservables(OutputWriter *ow) {
	for(unsigned d=0; d<real_database.size(); d++) {
		real_database[d]->setDBObservables(ow);
	}
}
