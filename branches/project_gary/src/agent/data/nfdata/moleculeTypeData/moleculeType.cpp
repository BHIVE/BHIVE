...
System * MoleculeType::getSystem() const { return system; }
...
int MoleculeType::createComplex(Molecule *m)
{ return system->createComplex(m); }
...
string MoleculeType::getComponentStateName(int cIndex, int cValue) {
	...
	if(cValue>(int)possibleCompStates.size() || cValue<0)
		return "?-value was: "+Util::toString(cValue);
...
void MoleculeType::outputMolObsNames(ofstream &fout)
{
	for(molObsIter = molObs.begin(); molObsIter != molObs.end(); molObsIter++ )
		fout<<"\t"<<(*molObsIter)->getName();
}

void MoleculeType::outputMolObsCounts(ofstream &fout)
{
	for(molObsIter = molObs.begin(); molObsIter != molObs.end(); molObsIter++ )
