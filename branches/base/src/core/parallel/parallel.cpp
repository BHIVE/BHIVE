...
void Hive::parallel_initialise(int argc, char *argv[], int& inrank, int& insize) {
#ifdef BHIVEMPI
    MPI_Init(&argc, &argv);
...
void Hive::parallel_pack_messages(vector<Message*> outbox, int& totalSize, char*& completeMessage) {
  ...
  //  cerr << "outboxsize" << outbox.size() << " " << Hive::mpirank() << endl;
    ...
    int current = 0;
    for (int i = 0; i < messageBlurb.size(); i++) {
...
//	map<int,vector<Message*> > cpuMessageMap;
//	for (int i=0; i < int(outbox.size()); i++) {
////		int dest = outbox[i]->getDestinationId();
//		int dest = outbox[i]->getDestinationNode();
//		cpuMessageMap[dest].push_back(outbox[i]);
//	}
//
...
//		//Sending and receiving messages
////		if (neighbors[i] < mpirank()) {
//		if (neighbors[i] < Hive::mpirank()) {
//		//	cerr << "a " << sendLength << endl;
//			parallel_send(neighbors[i], sendBuffer, sendLength);
//		//	cerr << "did sending " << Hive::mpirank() << endl;
//			parallel_recv(neighbors[i], recvBuffer, recvLength);
//		//	cerr << "did receive " << Hive::mpirank() << endl;
//		} else {
//		//	cerr << "b " << sendLength << endl;
//			parallel_recv(neighbors[i], recvBuffer, recvLength);
//		//	cerr << "did receive something" << Hive::mpirank() << endl;
//		//	cerr << "pre2 " << mpirank() << endl;
//			parallel_send(neighbors[i], sendBuffer, sendLength);
//		//	cerr << "did sending " << Hive::mpirank() << endl;
//		}
////		MPI_Barrier(MPI::COMM_WORLD);
//		delete [] sendBuffer;
////		cerr << "made it out 1 " << Hive::mpirank() << endl;
//		//Translating the received message into message objects
//		if (recvLength > 0 && recvBuffer != NULL) {
//			parallel_unpack_messages(inbox,recvLength,recvBuffer);
//		}
//	}
////	cerr << "leaving " << mpirank() << endl;
//}
//
//=======
//>>>>>>> .r295
...
//	cerr << "entering parallel unpack of messages" << endl;
	...
	while (currentLocation < recvLength) {
		//Creating a new message object to hold the message data
//<<<<<<< .mine
//		Message* NewMessage = new Message();
//		cerr << "in hier 1" << endl;
//		NewMessage->setDestinationID(((int*)currentBuffer)[0],mpirank());
