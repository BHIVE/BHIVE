...
namespace Hive {
	...
	/**
	 * @brief one dimensional double linear gradients
	 *
	 * at x=0, we have the maximum, which then decays linearly in either
	 * direction down to zero.  Provide the slope (positive slope has
	 * peak at zero, negative slope has min at zero) and the intercept
	 * concentration in molar
	 *
	 */
	class TriangleMathFunctionData : public MathFunctionData {
	public:
		TriangleMathFunctionData(string dataName):MathFunctionData(dataName){};
		virtual ~TriangleMathFunctionData() {};

		void printContent(ostream& out) { out << "blupdiblupp 2"; };

		double getValue(double x) {

			double ret = -slope*abs(x) + intercept;
			if (ret <=0)
				return 0;
			else
				return ret;
	...
	/**
		 * @brief one dimensional linear gradients with plateau
		 *
		 * at x=0, we have the maximum, which then decays linearly in the
		 * negative direction and stays level in the positive direction
		 *
		 */
		class NickGradMathFunctionData : public MathFunctionData {
		public:
			NickGradMathFunctionData(string dataName):MathFunctionData(dataName){
				this->length=0; this->slope=0; this->intercept=0;
			};
			virtual ~NickGradMathFunctionData() {};

			void printContent(ostream& out) { out << "blupdiblupp 2"; };

			double getValue(double x) {

				if(x>=0) { return this->intercept; }
			...
			void setLength(double l) {this->length = l; this->slope = this->intercept / this->length; };

			void setYIntercept(double i) {this->intercept = i; this->slope = this->intercept / this->length;};
	...
	public:
		...
		double getValue(vector<double>& pos, double t) {
			double fraction = initial/(sqrt(2.0*M_PI*6.0*diff*(t-t0)));
