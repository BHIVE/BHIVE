...
HeatbugComposer::HeatbugComposer(int argn, char *argv[]) : Composer() {
	...
	if (commandline.find("-gridsize") != commandline.end())
		this->grid_size = atoi(commandline["-gridsize"].c_str());
	else
		this->grid_size = 100;
...
double HeatbugComposer::getTimeIncrement() {
	Database *db = orchestra[0]->getDatabase();
	double dt = ((DoubleData *) db->getDataItem("dt"))->getDouble();
	return dt;
}
