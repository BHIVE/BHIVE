...
void Coordinate::setDomain(int d) {
	domain = d;
}

int Coordinate::getDomain() {
	return domain;
}
...
void NVoxel::printCenter() {
	double x = (this->grid_ptr->getVertex(vertices[0])->getX() +
		this->grid_ptr->getVertex(vertices[6])->getX())/2;
	double y = (this->grid_ptr->getVertex(vertices[0])->getY() +
			this->grid_ptr->getVertex(vertices[6])->getY())/2;
	double z = (this->grid_ptr->getVertex(vertices[0])->getZ() +
			this->grid_ptr->getVertex(vertices[6])->getZ())/2;
	cout << x << " " << y << " " << z << endl;
}
...
void NVoxel::getCoordinate(int i, Coordinate& c) {
	...
	if (i < vertices.size()) {
	  ...
	  x = this->grid_ptr->getVertex(vertices[i])->getX();
	  y = this->grid_ptr->getVertex(vertices[i])->getY();
	  z = this->grid_ptr->getVertex(vertices[i])->getZ();
	  // cout << x << " "<< y << " " << z << endl;
