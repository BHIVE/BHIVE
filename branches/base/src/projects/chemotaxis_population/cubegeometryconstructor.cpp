...
CubeGeometryConstructor::CubeGeometryConstructor() {
	cerr << "# entering CubeGeometryConstructor:CONSTRUCTOR" << endl;
	this->length = 0;
	this->stepsize = 0;
	this->number_coordinates = 0;
	this->number_points_height = 0;
	this->number_points_plane = 0;
	cerr << "# leaving CubeGeometryConstructor:CONSTRUCTOR" << endl;
}
...
Grid* CubeGeometryConstructor::returnGrid(string name) {
	...
	// this is not very nice and should be moved to the cube-geometry-constructor
	for (int i=0; i<grid->getNumberOfVoxels(); i++)
		grid->getVoxel(i)->setNumberSpecies(this->number_species);
	/// add names of species and their diffusion coefficients
	for (int j=0; j<this->number_species; j++)
		grid->addSpecies(this->species_names[j],this->diff_species[j]);
...
void CubeGeometryConstructor::setSpecies(int number, vector<string>& names, vector<double>& diffs) {
	this->number_species = number;
	this->species_names.resize(number);
	this->diff_species.resize(number);
	for (int i=0; i<number; i++) {
		this->species_names[i] = names[i];
		this->diff_species[i] = diffs[i];
	}
}
