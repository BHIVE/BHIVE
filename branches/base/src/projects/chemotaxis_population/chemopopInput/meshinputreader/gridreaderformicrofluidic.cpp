...
GridReaderForMicroFluidic::GridReaderForMicroFluidic() {
	this->inputset = false;
	this->g = new Grid();
	g->setIsQuadGrid(true);
	// don't need line below, cube grid is defaulted to false
	g->setIsCubeGrid(false);
	this->resolution = 0;
}
...
void GridReaderForMicroFluidic::setHeight(double h) {
	this->height = h;
}
...
void GridReaderForMicroFluidic::determineNeighborhood(vector<vector<int> >& voxels,
	...
	for (unsigned int i=0; i<voxel_lookup->size(); i++) {
		for (unsigned int j=0; j<voxel_lookup->at(i)->size(); j++) {
			...
			if (voxel_a != -1) {
				// lower x: j-1, upper x: j+1, lower y: i-1, upper y: i+1
				// j-1
				if (j > 0) {
					...
					} else neighborhood.at(voxel_a).push_back(-1);
				} else neighborhood.at(voxel_a).push_back(-1);
				// j+1
				...
				// add walls for lower and upper z walls as well
				neighborhood.at(voxel_a).push_back(-1);
				neighborhood.at(voxel_a).push_back(-1);
			}
		}
	}
