...
void GWAParser::parseEnvironment() {
	...
	while (getline(*input, line) && line != "END") {
		...
		if (line.substr(0,1) != "#" && line != "") {
			...
			else if (entype == "BOUNDARY")
				this->parseBoundaryBehavior();
...
void GWAParser::parseFlags() {
	...
	while (getline(*input, line) && line != "END") {
		...
		if (line.substr(0,1) != "#" && line != "") {
			...
			} else if (flagtype == "DIMENSION") { // parsing the celltype
				cerr<<"#    in GWAParser: parsing DIMENSION as: "<<flagvalue<<endl;
				this->setDimension(flagvalue);
			} else if (flagtype == "ROTDIF") {
				cerr<<"#    in GWAParser: parsing ROTDIF as: "<<flagvalue<<endl;
				this->setRotDif(flagvalue);
			} else if (flagtype == "CELLSPEED") {
				cerr<<"#    in GWAParser: parsing CELLSPEED as: "<<flagvalue<<endl;
				this->setCellSpeed(flagvalue);
			} else if (flagtype == "TUMBLESTYLE") {
				cerr<<"#    in GWAParser: parsing TUMBLESTYLE as: "<<flagvalue<<endl;
				this->setTumbleStyle(flagvalue);
			} else {
...
void GWAParser::setDimension(string value) {
	if (!db->doesDataItemExist("isIn2D")) {
		...
		if(value=="2D" || value=="2d") {
			...
			throw HiveException("# the DIMENSION flag must be either 2D or 3D", "GWAParser::setDimension()");
		}
...
void GWAParser::setRotDif(string value) {

	double rotDifNumber = 0;
	try {
		rotDifNumber = Util::convertToDouble(value);
	} catch (HiveException &e) {
		e.addTraceException("GWAParser::setRotDif(string value)");
		throw e;
	}
	...
	if (!db->doesDataItemExist("rotational_diffusion_constant")) {
		...
		DoubleData *rotdiffconst = new DoubleData("rotational_diffusion_constant",rotDifNumber);
		db->addData(rotdiffconst->getName(), rotdiffconst);
	} else {
		((DoubleData *)db->getDataItem("rotational_diffusion_constant"))->setDouble(rotDifNumber);
	}
}


void GWAParser::setCellSpeed(string value) {
	double cellSpeedNumber = 0;
	try {
		cellSpeedNumber = Util::convertToDouble(value);
	} catch (HiveException &e) {
		e.addTraceException("GWAParser::setCellSpeed(string value)");
		throw e;
	}

	// try to add cell speed variable to the world.  This will be accessed when the cell_speeds vector is created
	if (!db->doesDataItemExist("all_cell_speeds")) {
		// rotational diffusion constant of the cells
		DoubleData *cellspeed = new DoubleData ("all_cell_speeds", cellSpeedNumber);
		db->addData(cellspeed);
	} else {
		((DoubleData* )db->getDataItem("all_cell_speeds"))->setDouble(cellSpeedNumber);
	}
}

void GWAParser::setTumbleStyle(string value) {
	...
	if(value=="random") {
		...
		throw(HiveException("TUMBLESTYLE parameter must be either 'random' or 'gamma'.","GWAParser::setTumbleStyle(string value)"));
	}

	// try to add cell speed variable to the world.  This will be accessed when the cell_speeds vector is created
	if (!db->doesDataItemExist("tumble_style_use_gamma")) {
		// rotational diffusion constant of the cells
		BoolData *useGammaDistribution = new BoolData ("tumble_style_use_gamma", useGamma);
		db->addData(useGammaDistribution);
	} else {
		((BoolData* )db->getDataItem("tumble_style_use_gamma"))->setBool(useGamma);
	}
}
...
void GWAParser::parseMathEnvironment() {
	...
	TVectorData<SimpleBoundaryData* > *boundaries = new TVectorData<SimpleBoundaryData* >("boundaries", "tvector_boundariesdata");
	while (getline(*input, line)) {
		Util::trim(line);
		if(line == "END") { break; }
		if (line.substr(0,1) != "#" && line != "") {
			...
			if (environ_type == "boundary") {
				string coordinate; double position; int boundaryType;;
				iss >> coordinate; iss >> position; iss >> boundaryType;
				SimpleBoundaryData *boundary = new SimpleBoundaryData("boundary");
				if(coordinate.compare("x")==0)
					boundary->setCoordinate(0,position,boundaryType);
				else if(coordinate.compare("y")==0)
					boundary->setCoordinate(1,position,boundaryType);
				else if(coordinate.compare("z")==0)
					boundary->setCoordinate(2,position,boundaryType);
				else
				{
					cerr<<"problem when parsing boundary. could not recognize: '"<<coordinate<<"' string as x, y, or z."<<endl;
					exit(1);
				}

				boundaries->addElementToEnd(boundary);
			}


			///////////////////////////
			else if (environ_type == "linear") {
			...
			} else if (environ_type == "triangle") {
				double slope, yintercept;
				iss >> slope; iss >> yintercept;
				TriangleMathFunctionData *environT = new TriangleMathFunctionData("environment");
				environT->setSlope(slope);
				environT->setYIntercept(yintercept);
				environments->addElementToEnd(environT);
			} else if (environ_type == "nick") {
				double length, yintercept;
				iss >> length; iss >> yintercept;
				NickGradMathFunctionData *environN = new NickGradMathFunctionData("environment");
				environN->setLength(length);
				environN->setYIntercept(yintercept);
				environments->addElementToEnd(environN);

//				cout<<"\n\n\n\n===\n[";
//				for(double xpos=-3000;xpos<3000; xpos+=5)
//				{
//					cout<<xpos<<" "<<environN->getValue(xpos)<<"; ..."<<endl;
//				}
//				cout<<"];"<<endl;
//				exit(1);
	...
	while (getline(*input,line) && line != "END") {
		...
		if (line.substr(0,1) != "#" && line != "") {
			...
			if (type == "CUBE") {
				...
				// read the resolution and height of the grid and the filename from the file.
				double r, h; string filename;
				iss >> r; iss >> h; iss >> filename;
				...
				cerr<<"# Creating microfluidic world.  Filename: "<<filename<<endl;
				cerr<<"#    Resolution: "<< r << " micrometers"<<endl;
				cerr<<"#    Height: "<< h << " micrometers"<<endl;
				cerr<<"#    DiffConst: "<< D <<endl;
...
void GWAParser::parseBoundaryBehavior() {
	string line = "";
		istringstream iss;
		string boundary;
		// set the grid flag in the world agent
		...
		while (getline(*input,line)) {
			...
			if (line.substr(0,1) != "#" && line != "") {
				...
				iss.str(line);
				iss >> boundary;
				// reflective results in bouncing off the wall perfectly
				// simple_parallel is just turning parallel to the wall
				// run_with_wall is turning parallel to the wall with slight rotation
				//   to mimic the circles that bacteria make (based on 2006 Lauga paper)
				if(boundary == "REFLECTIVE") {
					StringData *s = new StringData("boundary_behavior","reflection");
					db->addData(s);
				} else if(boundary == "SIMPLE_PARALLEL") {
					StringData *s = new StringData("boundary_behavior","simple_parallel");
					db->addData(s);
				} else if(boundary == "RUN_WITH_WALL") {
					StringData *s = new StringData("boundary_behavior","run_with_wall");
					db->addData(s);
				} else
					throw HiveException("Do not understand boundary behavior to be parsed",
										"GWAParser::parseBoundaryBehavior");
			}
		}
}
...
//				cerr << x << " " << y << " " << z << " " << r << " " << number << endl;
			...
			} else if (type == "RANDOM_ALONG_X_AXIS") {
				...
				// nothing to be read
				cpi.initUniformDistAlongXAxis(this->ag,minX,maxX,number);
