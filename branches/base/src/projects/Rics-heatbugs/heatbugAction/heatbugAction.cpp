...
void SpecialAgentReceiveBugPositionsAction::prepareOutputfile(string name, bool bin) {
	if (bin) {
	...
	if (!this-isbinary) {
		for (int i=0; i<local_temp_grid->size(); i++) {
			for (int j=0; j<local_temp_grid->at(i)->size(); j++)
				*this->out << local_temp_grid->at(i)->at(j).getDouble() << " ";
				*this->out << endl;
		}
		*this->out << endl << endl;
	} else {
...
void SpecialAgentReceiveTemperatureFieldAction::prepareOutputfile(string name, bool bin) {
	if (bin) {
		this->out = new ofstream(name.c_str(),ios_base::binary);
		this->isbinary = true;
	} else
...
void SpecialAgentUpdateBugHappinessAction::fire(Data *d) {
	...
	if (this->number_received == this->number_bugs) {
		if (!isbinary)
			*this->out << this->number_outputs*this->output_interval << " " << this->acc_happiness/this->number_bugs << endl;
		else {
			double t = this->number_outputs*this->output_interval;
			double h = this->acc_happiness/this->number_bugs;
			this->out->write(reinterpret_cast<char *> (&t), sizeof(t));
			this->out->write(reinterpret_cast<char *> (&h), sizeof(h));
		}
...
void SpecialAgentUpdateBugHappinessAction::prepareOutputfile(string name, bool bin) {
	if (bin) {
		this->out = new ofstream(name.c_str(),ios_base::binary);
		this->isbinary = true;
	} else
