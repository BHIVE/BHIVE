...
{
	size_t id1 = original.find("(",0);
	size_t idx = original.find(compName,id1);
	if(idx!=string::npos) {
		//Only add it if it wasn't added before
		if(original.at(idx+compName.size())=='!')
			return original;
		return original.insert(idx+compName.size(),"!"+Util::toString(bondNumber));
	}
	return original+compName+"!"+Util::toString(bondNumber)+",";
}
		...
		for(int c=0; c<tm->n_symComps; c++)
		{
			...
			if(tm->symBondPartner[c]!=0) {
				...
				if(currentBondNumber!=-1) {
					...
					str+="!"+Util::toString(currentBondNumber);
				}
