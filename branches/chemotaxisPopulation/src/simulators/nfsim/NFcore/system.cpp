...
Observable * System::getObservable(int index) const
{
	return obsToOutput.at(index);
}


int System::getNumOfObservables() const
{
	return (int)obsToOutput.size();
}
...
double System::sim(double duration, long int sampleTimes, bool verbose)
{
	...
	while(current_time<end_time)
	{
		...
		if(a_tot>ATOT_TOLERANCE) delta_t = -log(Util::RANDOM_CLOSED()) / a_tot;
...
void System::outputAllPropensities(double time, int rxnFired)
{
	if(!propensityDumpStream.is_open()) {
		...
		for(unsigned int r=0; r<allReactions.size(); r++) {
			...
			for(int rl=0; rl<allReactions[r]->getNumOfReactants(); rl++) {
				propensityDumpStream<<" rL"<<Util::toString(rl);
			}
		}
	...
	for(unsigned int r=0; r<allReactions.size(); r++) {
		propensityDumpStream<<" ";
			...
			for(int rl=0; rl<allReactions[r]->getNumOfReactants(); rl++) {
				propensityDumpStream<<" "<<Util::toString((int)allReactions[r]->getReactantCount(rl));
		}
	}
	propensityDumpStream<<endl;
...
ofstream& System::getOutputFileStream()
{
