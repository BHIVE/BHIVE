...
void UpdateWorldBackgroundConcentrationAction::prepare() {
	// write out a warning:
	cerr << "# warning setting up UpdateWorldBackgroundConcentrationAction: " << endl
		 << "# assuming that the first element of the environments vector is of the type" << endl
		 << "# OneDLinearMathFunctionData. " << endl;
	cerr << "# if this is not the case, this will most likely lead to a segmentation fault" << endl;
	// check for environments vector
	if (!agent->getDatabase()->doesDataItemExist("environments"))
		throw HiveException("no environments in database", "UpdateWorldBackgroundConcentrationAction");
	// get environemts vector
	TVectorData<MathFunctionData*> *vec = (TVectorData<MathFunctionData*> *) this->agent->getDatabase()->getDataItem("environments");
	// add onedlinearmathfunction, if the environments vector is empty
	if (vec->size() == 0) {
		cerr << "# there is no mathfunction environment in the environments vector" << endl;
		cerr << "# adding a OneDLinearMathFunctionData with slope 0 and y-intercept 0" << endl;
		OneDLinearMathFunctionData *frank = new OneDLinearMathFunctionData("environment");
		frank->setSlope(0);
		frank->setYIntercept(0);
		vec->addElementToEnd(frank);
		// we need to add a birthtime to this environment.
		if (agent->getDatabase()->doesDataItemExist("birthtimes_of_environments")) {
			TVectorData<double> *bla = (TVectorData<double> *) agent->getDatabase()->getDataItem("birthtimes_of_environments");
			bla->addElementToEnd(0);
		} else {
			TVectorData<double> *bla = new TVectorData<double> ("birthtimes_of_environments", "tvector_double");
			bla->addElementToEnd(0);
			agent->getDatabase()->addData(bla);
		}

	}
	odlmfd = (OneDLinearMathFunctionData *) vec->at(0);
}
...
void UpdateCellSeenLigandAction::prepare() {
	...
	if(db->existsDataItem("essence")) {
		...
		cellaccligand = (TVectorData<double>* ) db->getDataItem("cell_acc_ligand");
	}
}
