...
bool NF::initParameters(TiXmlElement *pListOfParameters, System *s, map <string,double> &parameter, bool verbose)
{
	try {
				...
				pParamElement != 0; pParamElement = pParamElement->NextSiblingElement("Parameter"))
		{
			...
			double d = 0;
			try {
				d = Util::convertToDouble(paramValue);
			} catch (HiveException &e) {
				e.addTraceException("Parsing parameter: \""+paramName+"\", NF::initParameters()");
						...
						for ( pAlStates = pListOfAllowedStates->FirstChildElement("AllowedState"); pAlStates != 0; pAlStates = pAlStates->NextSiblingElement("AllowedState"))
						{
							...
							try {
								...
								if(aState!="PLUS" && aState!="MINUS") {
									stateVal = Util::convertToInt(aState);
								}
		...
		bool verbose)
{
	...
	try {
		...
		for ( pSpec = pListOfSpecies->FirstChildElement("Species"); pSpec != 0; pSpec = pSpec->NextSiblingElement("Species"))
		{
			...
			try {
				specCountInteger = Util::convertToInt(specCount);
			} catch (HiveException &e) {
					...
					if(finalState=="PLUS") {
						...
						try {
							...
							if(allowedStates.find(c->t->getMoleculeTypeName()+"_"+lookupname+"_"+finalState)==allowedStates.end()) {
								...
								//}
								//if(allowedStates.find(c->t->getMoleculeTypeName()+"_"+lookupname+"_"+finalState)==allowedStates.end()) {
									cout<<"Error! in NF, when looking up state: "<<c->t->getMoleculeTypeName()+"_"+c->symPermutationName+"_"+finalState<<endl;
							...
							if(productName==id) {
								TiXmlElement *pListOfMols = pProduct->FirstChildElement("ListOfMolecules");
									if(pListOfMols) {
										...
										bool ok = NF::readProductPattern(pListOfMols,s,parameter,allowedStates, productName,
				...
				if(!pRateLaw->Attribute("id") || !pRateLaw->Attribute("type")) {
					...
					if(rateLawType=="Ele")
					{
						...
						if(!pRateConstant) {
							...
							try {
								rate = Util::convertToDouble(rateValue);
							} catch (HiveException &e) {
						...
						if(!pListOfRateConstants) {
							...
							if(!pRateConstant) {
								...
								if(!pRateConstant->Attribute("value")) {
									...
									try {
										kcat = Util::convertToDouble(kcatName);
									} catch (HiveException &e) {
							...
							if(!pRateConstant) {
								...
								if(!pRateConstant->Attribute("value")) {
									...
									bool usedParam = false;
									try {
										Km = Util::convertToDouble(KmName);
									} catch (HiveException &e) {
...
bool NF::readObservableForTemplateMolecules(TiXmlElement *pObs,
	...
	for ( pPattern = pListOfPatterns->FirstChildElement("Pattern"); pPattern != 0; pPattern = pPattern->NextSiblingElement("Pattern"))
	{
		...
		if(pPattern->Attribute("relation")) {
			if(pPattern->Attribute("quantity")) {
				...
				try {
				quantity = Util::convertToInt(pPattern->Attribute("quantity"));
				} catch (HiveException &e) {
...
TemplateMolecule *NF::readPattern(
					...
					if(symMap.find(compId)==symMap.end() && moltype->isEquivalentComponent(compName)) {
						...
						if(pComp->Attribute("numberOfBonds")) {
							...
							if(numOfBonds.compare("+")==0) {
								...
								try {
									numOfBondsInt = Util::convertToInt(numOfBonds);
								} catch (HiveException &e) {
